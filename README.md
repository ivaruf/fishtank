...
### Clownfish quality study

The clownfish now has four authored models, switched live through Render quality:
...
Each tier has an editable `assets/blender/clownfish*.blend` source. Rebuild all four with `blender --background --factory-startup --python tools/blender/create_fish.py -- --models-only clownfish`. The detail layers live in `tools/blender/clownfish_detail.py`. These are geometric details and plain colored materials, without external textures. Geometry and materials remain shared between instances; each fish has its own swim animation phase. Studio comparisons are saved as `docs/clownfish-low.png`, `docs/clownfish.png`, `docs/clownfish-high.png`, and `docs/clownfish-hd.png`. Blue tang and pufferfish also have four authored tiers; the remaining species use their standard model for Battery saver/Balanced/Sharper and HD for Ultra. Higher tiers cost more geometry and pixels, so compare on your target device before choosing a default.

The blue tang adds swept sail fins, gill covers, scale arcs, bilateral fin rays and animated flippers. The pufferfish adds sculpted lips, gill crescents, belly ridges, tapered spines and fine freckles. Rebuild with the same command using `blue-tang pufferfish`; their detail layers live in `tools/blender/reef_detail.py`. Remaining work is tracked in [the fish upgrade worklist](docs/FISH-UPGRADE-WORKLIST.md).
