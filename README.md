...
The eight-player cap per tank is a bandwidth decision, not a gameplay or CPU one. A snapshot is roughly 25 KB and about 95% of it is the 100 wild fish, so each extra player adds only ~0.26 KB — but every client receives the whole snapshot at 15 Hz, so a full tank pushes ~3.2 MB/s (26 Mbit/s), about all a home upstream has. Three tanks cap the host near 78 Mbit/s. Server CPU is nowhere near the limit: a room-tick costs about 0.12 ms, so one core could sustain a few hundred active rooms. Culling distant wild fish from snapshots, rather than raising the cap, is the way to make rooms bigger.
