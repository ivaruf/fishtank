...
Tests cover eating thresholds/head collision, protection, growth, NPC movement, hunting and predation, kelp cover, the filter's arming delay, zap, recharge, stun and food chunks, respawn, PvP death and respawn, all tank boundaries, round transitions, HTTP assets including the `.glb` models, two real WebSocket clients, private solo rooms, malformed messages, authority, and disconnect/reconnect. `tests/fish.test.js` loads the real model files on Babylon's NullEngine and checks orientation, instancing, material conversion, the swim clip, bite and death animation state, threat tints through the per-instance color buffer, disposal, and the procedural fallback.
