...
- Its id space is shared with every other PeerJS app, so codes are namespaced
  ...
  A code is three picture names joined by dashes — `fishtank-v1-boat-fish-star`
  — which is 1,728 of them. Far fewer than the six-consonant codes it replaced,
  and deliberately so: only rooms open at the same moment can collide, there
  are a handful at most, and a taken code is reported rather than silently
  wrong. See `client/js/codes.js` for why the code is pictures and why three
  taps beat a bigger id space.
